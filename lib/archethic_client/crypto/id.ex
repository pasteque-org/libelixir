defmodule ArchethicClient.Crypto.ID do
  @moduledoc false
  ...
  def prepend_hash(hash, algorithm), do: <<from_hash(algorithm)::8, hash::binary>>

  @spec prepend_curve(binary(), integer) :: Crypto.address()
  def prepend_curve(hash, curve_type), do: <<curve_type::8, hash::binary>>
  ...
  def prepend_keypair({public_key, private_key}, curve, origin \\ :software),
    do: {prepend_key(public_key, curve, origin), prepend_key(private_key, curve, origin)}
